...
description: >
  ...
  greenfield or brownfield — into shape-notes.md, the input to /ai-prd.
  ...
  Use BEFORE /ai-prd, not in place of it.
...
# Shape: Facilitate Discovery (Greenfield & Brownfield) Before /ai-prd

This skill is the head of the bootstrap chain. For greenfield: `/ai-shape → /ai-prd → ai-tech-stack-selector → bootstrapper`. For brownfield: `/ai-shape → /ai-prd → ai-stack-assess → ai-health-check`. Its single job: walk a user from "I have an idea" (greenfield) or "I want to change this system" (brownfield) to a structured `context/foundation/shape-notes.md` that `/ai-prd` can turn into a PRD that conforms to the locked schema.
...
The locked schema both this skill and `/ai-prd` conform to lives at `references/prd-schema.md` (relative to this SKILL.md). Read it before producing any artifact and re-check against it at every checkpoint write.
...
**Skip when**: the project already has a PRD or an ADR set (use `/ai-frame` or `/ai-plan` instead), or the user is reasoning about a single bug / refactor / small feature within an existing codebase that doesn't warrant a full PRD (use `/ai-frame`). For brownfield projects where the user wants to shape a meaningful change, this skill IS the right starting point.

## Relationship to other skills

- `/ai-init` — scaffolds the `/context` skeleton (`changes/`, `archive/`, `foundation/`) plus universal READMEs in each. `/ai-shape` requires `context/foundation/` to exist; if absent, it delegates to `/ai-init` via the `Skill` tool (Step 0 below).
- `/ai-prd` — consumes `shape-notes.md`. The handoff is the `## Step 8` clipboard write.
- `/ai-frame` — for *reframing* small-scope problems within existing systems where a full PRD is overkill. `/ai-shape` is for larger brownfield changes (new modules, significant features) that need structured discovery and a PRD.
- `/ai-stack-assess` — downstream of `/ai-prd` for brownfield projects. Evaluates existing stack against quality gates.
- `/ai-health-check` — downstream of `/ai-stack-assess` for brownfield. Audits existing project health.
- `/ai-plan` — downstream of `/ai-prd`, never invoked from here directly.

## Initial Response

When this skill is invoked:

1. **If a freeform idea was provided as the argument** (e.g. `/ai-shape a recipe app that suggests meals from what's in your fridge`), capture it verbatim as the **seed idea**. Do not rephrase. Proceed to Step 0.
2. **If a file path was provided** (e.g. `/ai-shape @notes/idea.md`), read it FULLY and use its contents as the seed. Proceed to Step 0.
3. **If nothing was provided**, respond with:

```
I'll help you shape an idea into structured notes that /ai-prd can turn into
...
Tip: pass the idea inline — `/ai-shape a recipe app that uses fridge contents`
     or for brownfield — `/ai-shape add a recommendation engine to my recipe app`
```
...
AskUserQuestion:
- question: "This directory isn't initialized for 10xWorkflow (context/foundation/ is missing). Run /ai-init now?"
  header: "Init?"
  options:
  - label: "Yes — run /ai-init (Recommended)"
...
On "Yes": invoke `/ai-init` via the **Skill** tool (NOT via Bash). When `/ai-init` returns, re-check the precondition; if it now passes, continue to Step 0.5. On "No": print "Stopping. Run `/ai-init` when ready, then re-invoke `/ai-shape`." and STOP.

Do not duplicate `/ai-init`'s scaffold logic. The `Skill` tool is the correct delegation path.
...
Write the confirmed `context_type` into the shape-notes.md frontmatter (alongside `checkpoint:`) immediately. This value is load-bearing for `/ai-prd`'s auto-routing.
...
- NEVER pre-commit to a stack (framework, database, hosting platform, language family). The PRD captures product-level priors only — `product_type`, `target_scale`, `timeline_budget`. Stack-shaped concerns are gathered downstream of `/ai-prd`.
...
- question: "How would you like to proceed?"
  ...
  - label: "Accept and finish"
    description: "Proceed despite the gaps. They will be recorded as warnings in the checkpoint and surfaced in /ai-prd's Open Questions."
...
```bash
echo -n "/ai-prd" | pbcopy 2>/dev/null || echo -n "/ai-prd" | clip.exe 2>/dev/null || echo -n "/ai-prd" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/ai-prd"
```
...
═══════════════════════════════════════════════════════════
  ...
  ► Next:   /ai-prd  (✓ copied to clipboard)

  After /ai-prd, the next chain step will pick up:
...
STOP. Do not chain into `/ai-prd` automatically — the user runs it when ready.
...
5. **Soft gate, not hard gate.** The closing cross-check WARNS but allows the user to override every gap. Override paths are recorded in the checkpoint as `quality_check_status: warned` and surfaced in `/ai-prd`'s `## Open Questions`. Refusing to finish is not in scope.

6. **Mode-aware behavior.** The skill auto-detects context type (greenfield vs brownfield) from project markers in cwd and adapts all six discovery phases accordingly. For brownfield, the discovery loop shifts from "what are you building from scratch?" to "what exists, what's changing, what must be preserved?". If the user invokes this skill for a small-scope problem within an existing codebase (single bug, quick refactor), suggest `/ai-frame` instead — `/ai-shape` is for changes that warrant a full PRD.
...
- This is a **shaping** skill. Output is `shape-notes.md`, not `prd.md`. `/ai-prd` is the document generator.
