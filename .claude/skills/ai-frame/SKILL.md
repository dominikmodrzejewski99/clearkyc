...
description: >
  ...
  the assumption", "rethink", "before I plan". Use BEFORE /ai-plan, not in
  place of it.
allowed-tools:
  - Read
  - Glob
...
The shape this skill addresses is general: the user describes something (an observation, a perceived problem, a scope they want to take on) and proposes a response (a cause, an approach, a plan structure) in the same breath. The two get treated as one fact. A perfect /ai-plan then ships a perfect solution — and the actual problem remains, because the framing was wrong; the plan was right; the user lost the day.

This skill is the framing step. /ai-plan answers *how to build it*. /ai-frame answers *what's actually the right thing to plan*.

## When to use, when to skip

**Use when**: input has bug-shape ("X is broken, let's build Y"), scope-shape ("we should split this into two plans", "is this even the right scope"), design-shape ("which approach do we even want"), or assumption-shape ("we're assuming X — is that true?"). Use also when stakes are high, when the system is unfamiliar to the user, or when /ai-plan is about to start on a task that smells like a stated cause rather than a verified one.
...
- `/ai-research` — broad codebase exploration. Frame can consume a research doc as input but does not replace it.
- `/ai-plan` — accepts frame output as input. The Frame Brief IS a valid first argument to /ai-plan.
- `/ai-plan-review` — validates an existing plan. Frame validates the *premise* before a plan exists.

The Frame Brief is useful standalone (as a discussion artifact, or to scope a quick fix) — it does not require /ai-plan to follow.

## Initial Response

When this skill is invoked:

1. **If a file path or change-id was provided** (e.g. `/ai-frame @context/changes/foo/research.md` or `/ai-frame foo`), resolve it: a `<change-id>` resolves to `context/changes/<change-id>/research.md` (read it if present). Read the file FULLY and proceed to Step 1.
...
Tip: pass research directly — `/ai-frame @context/changes/<change-id>/research.md` (or just `<change-id>`)
```
...
Use AskUserQuestion. **The questions and options here are fundamentally different from /ai-plan's**: in /ai-plan, options are *solution choices*; here, options are *hypothesis disambiguators*. The user's answer narrows the hypothesis space.
...
- If invoked as `/ai-frame <change-id>` and `context/changes/<change-id>/` exists, write into it.
- Otherwise derive a kebab-case `<change-id>` from the observation and create the folder + `change.md` (mirroring `/ai-new` semantics) before writing.
...
[Pick one. If LOW, list the specific verification step needed before /ai-plan.]
...
- question: "Frame done. How would you like to proceed?"
  header: "Next step"
  options:
  - label: "Hand off to /ai-plan"
    description: "Pass this brief to /ai-plan and start implementation planning."
...
If the user picks "Hand off to /ai-plan", copy the command to clipboard:

```bash
echo -n "/ai-plan <change-id>" | pbcopy 2>/dev/null || echo -n "/ai-plan <change-id>" | clip.exe 2>/dev/null || echo -n "/ai-plan <change-id>" | xclip -selection clipboard 2>/dev/null || true
```

```powershell
# PowerShell (Windows)
Set-Clipboard "/ai-plan <change-id>"
```

And print: `→ /ai-plan <change-id> (✓ copied)`
...
2. **Observation and stated cause stay separate.** Through every step. The Frame Brief preserves the original framing verbatim — even when reframed — because future readers (and /ai-plan-review) need to see what was assumed vs what was discovered.

3. **No solution design.** This skill never picks an implementation approach. It does not propose phases, file changes, or technical decisions. It produces ONE artifact: the reframed (or confirmed) problem statement. /ai-plan owns the solution.

4. **Narrowing questions ≠ solution questions.** /ai-plan asks "which approach?". /ai-frame asks "where in the dimension map does the actual problem live?". This rule binds both Step 1.5 (pre-dispatch scope/observation narrowing) and Step 4 (post-dispatch hypothesis narrowing). Options describe observations or design positions, not choices about how to address them. If you find yourself drafting a question whose answer changes the *direction*, you've crossed into /ai-plan territory — stop.
...
- The Frame Brief is the only artifact. Keep it short, scannable, and actionable for /ai-plan.
